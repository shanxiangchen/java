...
<body>
	<form  action="updateFiledType.html" enctype="multipart/form-data" name="addFiledTypeForm" id="addFiledTypeForm" target="result"  method="post">
	 ...
	 <table border="0" cellpadding="0" cellspacing="0"> 	
		...
		<c:forEach items="${baseList}" var="li">
		  <tr class="info">
		     <td>
		         <input name="test" <c:if test="${not empty li.SORTNUM}">checked="checked"</c:if> type="checkbox"/>&nbsp;&nbsp;
		     ...
		     <td style="text-align:center;"><input type="text" value="${li.SORTNUM}" maxlength="5" style="width:30px;" name="sortNos"  ></td>
		     ...
		     <td style="text-align:center;">
		       <c:if test="${li.FILEDABCNAME=='FILED_ALERT_INFO'}">
		          <input type="text" style="width:250px;" maxlength="250" value="${li.FILEDALERT}" name="filedAlertInfo"   >
		       </c:if>
		     </td>
		   </tr> 
		</c:forEach>
		  ...
		  <tr class="info">
		     <td>
		        <c:choose>
		            <c:when test="${li.FILEDABCNAME=='YES_AND_NO'}">
		              <input id="yesAndNo" <c:if test="${not empty li.SORTNUM}">checked="checked"</c:if> type="checkbox"/>&nbsp;&nbsp;
		            </c:when>
		            <c:otherwise>
		               <input <c:if test="${not empty li.SORTNUM}">checked="checked"</c:if>  type="checkbox"/>&nbsp;&nbsp;
		            </c:otherwise>
		        </c:choose>
		        <input type="hidden" name="filedIds" value="${li.FILEDID}"/>
		        <input type="hidden" name="filedAbcNames" value="${li.FILEDABCNAME}"/>
		     ...
		     <td style="text-align:center;">
		       <c:choose>
		         ...
		         <c:otherwise>
		              <input type="text" maxlength="5" value="${li.SORTNUM}" style="width:30px;" name="sortNos"  >
		         </c:otherwise>
		       </c:choose>
		     </td>
		     <td style="text-align:center;">
		       <c:choose>
		          <c:when test="${li.FILEDFORMTYPE=='10'}">
		            <input type="text" maxlength="5" style="width:30px;" value="${li.MAXLENGTH}" name="maxlengths"  > 
		         </c:when>
		         <c:otherwise>
		             <input type="hidden" maxlength="5" value="0" style="width:30px;" name="maxlengths"  > 
		         </c:otherwise>
		     ...
		     <td style="text-align:center">
		       <c:choose>
		         ...
		         <c:otherwise>
		            <select name="isShows" >
				      <option <c:if test="${li.ISSHOW=='0'}">selected</c:if> value="0">默认展示</option>
				      <option <c:if test="${li.ISSHOW=='1'}">selected</c:if> value="1">选是展示</option>
				      <option <c:if test="${li.ISSHOW=='2'}">selected</c:if> value="2">选否展示</option>
			        </select>
		         </c:otherwise>
		       </c:choose>
		     </td>
		     <td style="text-align:center;">
		       <c:choose>
		         <c:when test="${li.FILEDABCNAME=='ALERT_INFO_ONE'}">
		            <input type="text"  maxlength="250" value="${li.FILEDALERT}" style="width:250px;" name="alertInfoOne"   >
		         </c:when>
		          <c:when test="${li.FILEDABCNAME=='ALERT_INFO_TWO'}">
		            <input type="text"  maxlength="250" value="${li.FILEDALERT}" style="width:250px;" name="alertInfoTwo"    >
		         </c:when>
		          <c:when test="${li.FILEDABCNAME=='ALERT_INFO_THREE'}">
		            <input type="text"  maxlength="250" value="${li.FILEDALERT}" style="width:250px;" name="alertInfoThree"   >
		         </c:when>
		         <c:when test="${li.FILEDABCNAME=='SERVICE_ALERT_INFO'}">
		            <input type="text"  maxlength="250" value="${li.FILEDALERT}" style="width:250px;" name="serviceAlertInfo"   >
		         </c:when>
		       </c:choose>
		     </td>
		   </tr> 
...
<script type="text/javascript">
       ...
       $("input[name='typeId']").attr("disabled",false);
...
/////////////////////////////////////////////互斥校验/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	    $("input[name='filedAbcNames'][value='MY_OF_TIMES']").prevAll("input[type='checkbox']").each(function(){
	           if($(this).attr("checked")){
	    ...
	    $("input[name='filedAbcNames'][value='DATE_OF_EQUITY']").prevAll("input[type='checkbox']").each(function(){
	          if($(this).attr("checked")){
	    ...
	    $("input[name='filedAbcNames'][value='DATE_OF_ENTRY']").prevAll("input[type='checkbox']").each(function(){
	          if($(this).attr("checked")){
	    ...
	    $("input[name='filedAbcNames'][value='DATE_OF_APPEARANCE']").prevAll("input[type='checkbox']").each(function(){
	          if($(this).attr("checked")){
